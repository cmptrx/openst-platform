...
baseInteract.prototype = {
  ...
  estimateGasForSignedTransaction: function() {
    ...
    if (_isValidSignedTransaction(signedTx)) {
      var responseData = responseHelper.error('l_w_i_3', "Invalid format for signedTx");
      return Promise.resolve(responseData);
    }
  ...
  callTransaction: function(rawTx) {
    ...
    if (!rawTx || Array.isArray(rawTx) || rawTx.constructor != Object) {
      var responseData = responseHelper.error('r_wi_3', "Invalid format for rawTx");
      return Promise.resolve(responseData);
    }

    if (!rawTx.from || !rawTx.to) {
      var responseData = responseHelper.error('r_wi_4', "Mandatory keys missing in rawTx");
      return Promise.resolve(responseData);
    }
  ...
  sendSignedTransaction: function(signedTx) {
    ...
    if (_isValidSignedTransaction(signedTx)) {
      return responseHelper.error('l_w_i_2', "Invalid format for signedTx");
    }
